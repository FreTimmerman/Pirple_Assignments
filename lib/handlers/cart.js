    ...
    if (userEmail) {
      ...
      _data.read('cart', userEmail, (err, cartdata) => {
        if (err) { //if no active cart, continue
          ...
          _data.read('menu', name, (err, menudata) => {
...
// Cart - PUT - changes the cart
// Required data: token, [menu-items,remark]
// Optional data: remark (inside array above)
_cart.put = (data, callback) => {
  let token = typeof (data.headers.token) == 'string' ? data.headers.token : false;
  let items = Array.isArray(data.payload.items) && data.payload.items.length > 0 ? data.payload.items : false;
  // Verify that the given token is valid for the email address
  ...
  if (items) {
    _tokens.verifyToken(token, userEmail => {
      if (userEmail) {
        ...
        _data.read('cart', userEmail, (err, cartdata) => {
          ...
          // only continue if it's an active cart
          if (!err) {
            // but empty the cart anyway, because this PUT is changing everything in the cart
            cartdata = {
              items: [],
              total: 0,
              waitTime: 0
            ...
            for (let item of items) {
              ...
              //try to read the item from the menu
              _data.read('menu', item.name, (err, menudata) => {
                counter++;
                if (err) { //it does not exist
                  errReason += item + " does not exist; ";
                } else { //it exists, continue
                  //check if the item is available
                  if (menudata.available) {
                    let newitem = {};
                    ...
                    errReason += item + " is not available; ";
                  }
                }
                //if this was the last item, perform the update. we have to do it in this loop because of race condition. (callbacks -_-)
                if (counter === items.length) {
                  ...
                  if (errReason.length > 0) {
                    ...
                    //all items in the cart exist and are available, update the cart
                    _data.update('cart', userEmail, cartdata, err => { //add to the cart with email as identifier
                      if (err) {
                        callback(500, {
    ...
    callback(400, { "Error": "no items array specified" })
  }
}
