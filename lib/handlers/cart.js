          ...
          _data.read('menu', item, (err, menudata) => {
            if (err) { //it does not exist
              ...
              if (menudata.available) {
                ...
                //console.log(newcart);
                _data.create('cart', token, newcart, err => { //create the cart with token as identifier
                  if (err) {
                    callback(500, {
          ...
          _data.read('menu', item, (err, menudata) => {
            if (err) { //it does not exist
              ...
              if (menudata.available) {
                ...
                _data.update('cart', token, cartdata, err => { //create the cart with token as identifier
                  if (err) {
                    callback(500, {
...
_cart.delete = (data, callback) => {
  ...
  _tokens.verifyToken(token, isTokenValid => {
    if (isTokenValid) {
      // Lookup the token
      _data.read('cart', token, (err, tokenData) => {
        if (!err && tokenData) {
          // Delete the token
          _data.delete('cart', token, err => {
            if (!err) {
