...
import { createRandomString, parseJsonToObject } from '../helpers.js';
...
// Orders - post
// Required data (as payload): paymentIntentID (string) 
// Required header: token
// Optional data (as payload): remark (string)
_order.post = (data, callback) => {
  let token = typeof (data.headers.token) == 'string' ? data.headers.token : false;
  let paymentIntentID = typeof (data.payload.paymentIntentID) == 'string' ? data.payload.paymentIntentID : false;
  let remark = typeof (data.payload.remark) == 'string' && data.payload.remark.length > 0 ? data.payload.remark : "no remarks";
      ...
      if (userEmail) {
        ...
        _data.read('cart', userEmail, (err, cartdata) => {
          if (!err) { //if no error (active cart is found), continue
            ...
            // Set up the request
            const stripePostReq = https.request('https://api.stripe.com/v1/payment_intents/' + paymentIntentID, stripePostOptions, function (res) {
...
_order.get = (data, callback) => {
  ...
  //let amount = typeof (data.payload.amount) == 'string' ? data.payload.amount : false; // TODO read this from the cartdata instead
  if (token) {
    // Verify that the given token is valid for the email address
    _tokens.verifyToken(token, userEmail => {
      if (userEmail) {
        ...
        _data.read('cart', userEmail, (err, cartdata) => {
          if (!err) { //if no error (active cart is found), continue
            ...
            const amount = cartdata.total * 100; //amount in eurocents
            // Set up the request
            const stripeDataString = `amount=${amount}&currency=eur`;
            const stripePostOptions = {
              port: '443',
              method: 'POST',
              auth: config.stripeSecretKey + ':'
            ...
            const stripePostReq = https.request('https://api.stripe.com/v1/payment_intents', stripePostOptions, function (res) {
              ...
              res.on('end', () => {
                if (res.statusCode == 200) {
                  ...
                  callback(200, { "client_secret": parseJsonToObject(data).client_secret })
                }
              });
            });
            // post the stripe data
